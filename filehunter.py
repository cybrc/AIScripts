...
def find_specific_files(unc_path, output_file, filenames):
    """Find specific filenames in the UNC path and append to the output file if not already present."""
    ...
    for root, dirs, files in os.walk(unc_path, topdown=True):
        ...
        # Loop through each specific filename and check if it exists in the current directory
        for file in files:
...
output_dir = "FileHunter"  # Folder to store the output text file
...
# Define the specific filenames you want to search for
filenames = ['password.txt', 'important.docx', 'confidential.pdf']  # Add more filenames as needed
...
while True:
    ...
    try:
        if find_specific_files(unc_path, output_file, filenames):
            print(f"Previous scan completed. New results appended to {output_file}\n")
        else:
            print(f"No new files with filenames {filenames} found in the specified path.\n")
